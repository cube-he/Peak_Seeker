...
model StudentProfile {
  ...
  serviceYear         Int     @default(2026)  // 服务年份（多年数据隔离）
...
阶段1 (4月14日 ~ 5月4日) — 3周 — 地基层
  Week 1: 数据库Schema升级（User拆分+Profile+分步迁移） + CASL权限体系
         + CI/CD流水线搭建 + 数据库备份脚本
  Week 2: 数据导入管线（事务+批量+验证报告） + Bull队列基础设施 + SSE通知
         + 推荐引擎单元测试框架搭建
  Week 3: 基础API（用户CRUD、学生管理、批次配置） + 前端路由架构（Route Groups）
         + API安全防护（限流+文件校验+JWT刷新）

阶段2 (5月5日 ~ 5月25日) — 3周 — 核心引擎+老师端
  Week 4: 推荐引擎核心（筛选→评分→系统抽样→去重）+ Bull异步执行
         + 引擎单元测试（选科匹配100%覆盖+评分+去重）
  Week 5: 推荐引擎完善（干净度+理由+风险+征集+趋势+多批次+降级策略）
         + 备选池Redis缓存 + 三层数据缓存 + Redis降级策略
  Week 6: 老师工作台前端（方案编辑器useReducer+撤销重做+看板+审核）
         + 批量导入学生 + 方案分享链接

阶段3 (5月26日 ~ 6月8日) — 2周 — 学生端+管理端+收尾
  Week 7: 学生端 + 管理员后台 + 算法参数调优工作流
         + 导出三格式（Excel完整/精简 + PDF打印版）
  Week 8: 集成测试 + 冒烟测试5条链路 + 监控部署（Winston+Bull Dashboard+Health）
         + 高考填报冻结期协议 + 上线

阶段4 (6月9日 ~ 6月20日) — 缓冲
  bug修复 + 性能优化 + 出分后批量重新生成验证
  填报截图/录取追踪/方案评估
  冻结前最后一次全功能冒烟测试
...
一、单元测试（Jest）
  ...
  选科匹配专项测试：
    穷举12种选科组合 × 所有选科要求类型 = 60+ 用例，必须100%通过

二、集成测试（Jest + Prisma test utils）
  API权限校验 / 方案状态流转全链路 / 数据导入事务 / Bull队列完成链路
  使用测试数据库，每个测试前重置
...
CI/CD（GitHub Actions）：
  push → lint → test → build → deploy staging（自动）
  production 部署需手动审批

版本化：
  每次部署打tag v2026.MMDD.N，Docker镜像同步打tag

快速回滚（<5分钟）：
  docker pull 上一个tag → docker-compose up
  关键迁移附带手写反向SQL（/migrations/rollback/）

高考填报期间冻结（6月20日~7月5日）：
  代码冻结：只允许hotfix（主管审批）
  数据冻结：只允许增量更新
  每个hotfix：修复→测试→staging验证→production→冒烟测试
```

### 20.3 方案编辑器状态管理

```
采用 useReducer + Context 独立管理，不依赖 Zustand

核心能力：
  - 撤销/重做栈（最多50步，每步带操作描述标签）
  - Ctrl+Z / Ctrl+Shift+Z
  - 离开保护（isDirty → "有未保存的修改"）
  - 乐观锁冲突检测与处理
  - 可选：操作历史面板（侧边栏展示操作时间线）

状态结构：
  items(当前志愿) + history(撤销栈) + future(重做栈)
  + historyLabels(操作描述) + isDirty + dataVersion
...
```
Bull Worker 并发数：4（单进程）
单任务平均 5秒 → 0.8 任务/秒
2700个任务 → 约56分钟（出分后批量场景可接受）

优化手段（如需加速）：
  A. concurrency 提升到 8-16（受CPU/内存限制）
  B. 同科类学生共享候选池筛选结果（省40%时间）
  C. 分批提交（每批10个）避免队列堆积
...
```
失败分类处理：
  TRANSIENT（暂时性）→ 自动重试3次，指数退避
  DATA_ERROR（数据问题）→ 通知老师 + 具体原因 + 操作建议链接
  ALGORITHM_ERROR（候选不足）→ 通知老师 + 建议放宽条件
  SYSTEM_ERROR（未知）→ 通知老师+管理员

批量生成进度面板：
  实时进度条 + 成功/失败/进行中计数
  失败项列出具体原因和操作按钮（查看/调整参数/重试）

批次跟踪：batchId → Redis存储进度 {total, completed, failed, failedJobs}
```

### 20.6 数据导入后验证机制

```
导入完成后自动运行验证报告（不直接提示"成功"）：

6项自动检查：
  1. 院校总数合理性（2000-3000范围）
  2. 各年份数据量对比
  3. 随机抽样10条（人工核对）
  4. 异常值检测（位次<0或>300000）
  5. 编码唯一性
  6. 与上次导入的差异量（删除>100条则警告）
...
```
隔离：StudentProfile.serviceYear 区分年份，默认查当年
归档：每年9月，当年学生 status→ARCHIVED，方案冻结
院校/专业库：累积模式（upsert），不按年重建
保留策略：
  学生信息：serviceYear+3年后脱敏
  方案/评估：永久保留（算法回测用）
  文件：serviceYear+1年后可清理
```

### 20.8 算法参数调优工作流

```
新增模型 AlgorithmConfig（已在十九章定义）：
  版本化参数配置，同一时间只有一个 isActive

调参流程：
  修改参数 → 选测试样本学生 → 新旧参数方案对比预览
  → 确认效果 → "应用参数"（isActive切换）
  → 旧配置自动保留（可回滚）

对比预览展示：
  哪些志愿变了、梯度分布变化、985/211占比变化、平均征集率变化
```
...
```
导出三种格式：
  Excel完整版（A3横版，24列，工作用）
  Excel精简版（A4，12列：序号/梯度/院校/专业/位次/计划/标签/性质/调剂/理由/风险）
  PDF打印版（A4纵版，灰度友好色，12pt+字号，页眉学生信息，页脚免责声明）

老师点"导出" → 选择格式 → Bull队列后端生成 → 通知下载
```
...
```
系统全局配置：
  freezeMode: boolean  // 管理员手动开启
  freezeStartDate / freezeEndDate

冻结模式下：
  ✅ 允许：方案生成/编辑/审核/导出/查看（核心业务不受影响）
  ✅ 允许：增量数据更新（招生计划变更）
  ❌ 禁止：全量数据重导入
  ❌ 禁止：数据库迁移
  ❌ 禁止：算法参数变更（除管理员+主管双重确认）
  ⚠️ 代码部署：仅hotfix，需主管审批

管理员 Dashboard 显示冻结状态横幅：
  "🔒 当前处于高考填报冻结期(6/20-7/5)，仅允许紧急操作"
```

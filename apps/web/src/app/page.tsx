    ...
    <MainLayout noPadding>
        ...
        <div className="relative max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-12 py-16 sm:py-20 lg:py-[110px]">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div className="hidden lg:block relative h-[420px]">
              <div className="absolute top-0 left-5 right-5 bg-white/90 backdrop-blur rounded-xl p-5 shadow-lg">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1.5">
                    <span className="bg-rush/10 text-rush text-xs font-medium px-2.5 py-1 rounded-full">冲12</span>
                    <span className="bg-stable/10 text-stable text-xs font-medium px-2.5 py-1 rounded-full">稳20</span>
                    <span className="bg-safe/10 text-safe text-xs font-medium px-2.5 py-1 rounded-full">保16</span>
                  </div>
                </div>
              ...
              <div className="absolute top-[160px] left-0 w-[48%] bg-white/90 backdrop-blur rounded-xl p-5 shadow-lg">
                <div className="text-xs text-text-muted">AI 精准度</div>
                <div className="font-serif text-[32px] font-semibold text-accent tabular-nums">99.8%</div>
                <div className="text-xs text-text-muted mt-1">历年预测准确率</div>
              </div>
              <div className="absolute top-[300px] left-[15%] right-[15%] bg-white/90 backdrop-blur rounded-xl p-5 shadow-lg">
                <div className="text-xs text-text-muted">数据覆盖</div>
                <div className="font-serif text-[32px] font-semibold text-text tabular-nums">全国在川招生</div>
      ...
      <section className="bg-surface-dim py-12 sm:py-16 lg:py-20 px-4 sm:px-6 lg:px-12">
        <div className="max-w-[1200px] mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-10">
            {[
              { icon: '🏛', bg: 'bg-primary-fixed', title: '院校全景', desc: '覆盖全国 2,800+ 院校，多维度数据横向对比，助你快速锁定目标。' },
              { icon: '📊', bg: 'bg-safe-fixed', title: '专业洞察', desc: '1,200+ 专业的就业前景、薪资走势与发展空间，看清每一条赛道。' },
              { icon: '✨', bg: 'bg-accent-fixed', title: 'AI 智能推荐', desc: '深度分析 15 年录取数据，结合你的分数与偏好，生成个性化冲/稳/保方案。' },
              { icon: '📈', bg: 'bg-rush-fixed', title: '趋势分析', desc: '录取分数线走势、报考热度变化、专业冷热轮动，数据驱动前瞻决策。' },
              { icon: '📋', bg: 'bg-stable-fixed', title: '方案管理', desc: '对比、调整、优化你的志愿组合，一键导出完整方案，填报时胸有成竹。' },
              { icon: '🔒', bg: 'bg-surface-dim', title: '隐私优先', desc: '企业级数据加密，你的个人信息与成绩数据始终在严格保护之中。' },
            ].map((card, i) => (
              <div
                key={i}
      ...
      <section className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-12 py-12 sm:py-16 lg:py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
          {[
            { num: '01', title: '输入你的成绩', desc: '填写高考分数、位次、所在省份。支持一分一段表精确定位。' },
            { num: '02', title: 'AI 生成方案', desc: '智能引擎分析 15 年录取数据，结合你的偏好生成个性化冲/稳/保方案。' },
            { num: '03', title: '优化并导出', desc: '调整、比较、确认。导出最终方案，胸有成竹走进填报系统。' },
          ].map((step, i) => (
            <div key={i}>

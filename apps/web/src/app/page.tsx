...
const features = [
  ...
  },
  {
    ...
    bg: 'bg-primary-fixed',
    title: '专业库',
    desc: '1,434 个本专科专业，结合历年录取数据与专业方向辅助横向比较。',
    href: '/majors',
    action: '浏览专业',
  },
  {
  ...
  return (
    <MainLayout noPadding hideMobileBottomNav>
      <section className="relative isolate overflow-hidden bg-[#1a2c47] text-white">
        ...
        />
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(800px_400px_at_70%_30%,rgba(184,134,11,0.18),transparent_60%),radial-gradient(600px_380px_at_15%_80%,rgba(44,82,130,0.50),transparent_60%)]" />

        <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-10 px-4 py-20 sm:px-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)] lg:gap-[60px] lg:px-8 lg:py-[80px]">
          <div>
            <CountdownBadge events={events} />
            <div className="mb-3.5 text-[11px] font-medium uppercase tracking-[2px] text-accent-light">
              为四川考生而生 · FOR SICHUAN STUDENTS
            </div>
            <h1 className="font-serif text-[40px] font-bold leading-[1.1] tracking-[-0.012em] text-white sm:text-[48px] lg:text-[56px]">
              每一个志愿，
              <br />
              都值得被认真对待。
            </h1>
            <p className="mt-[18px] max-w-[540px] text-[18px] leading-[1.65] text-white/75">
              汇集 2022-2025 年四川在川招生录取数据，结合 AI 智能推荐与 4 年位次趋势，帮助你做出更有把握的志愿决策。
            </p>

            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Link href="/recommend" className="btn-willnest btn-accent-willnest btn-lg-willnest">
                免费开始智能推荐 →
              </Link>
              <Link href="/universities" className="btn-willnest btn-onnav-willnest btn-lg-willnest">
                浏览在川招生院校
              </Link>
            </div>

            <div className="mt-10 flex flex-wrap gap-x-8 gap-y-4 text-[13px] text-white/60">
              {stats.map((item) => (
                <div key={item.label}>
                  <span className="mb-0.5 block font-serif text-2xl font-semibold tabular-nums text-white">
                    {item.value}
                  </span>
                  {item.label}
                </div>
          ...
          <div className="hidden rounded-[14px] border border-white/[0.12] bg-white/[0.06] p-6 text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_24px_70px_rgba(0,0,0,0.18)] backdrop-blur-xl md:block">
            <div className="text-[11px] uppercase tracking-[1.6px] text-white/55">三步预估你的位次</div>
            <h2 className="mt-1.5 font-serif text-[22px] font-semibold leading-tight text-white">
              输入分数，看看你能去哪。
            </h2>

            <div className="mt-[18px] border-b border-white/15 pb-2.5">
              <div className="text-[11px] uppercase tracking-[1.5px] text-white/55">2026 年高考分数</div>
              <div className="mt-1.5 flex items-baseline gap-2.5">
                <span className="font-serif text-[42px] font-bold leading-none tabular-nums text-white">628</span>
                <span className="text-[13px] text-white/55">/ 750 · 物理类</span>
              </div>
            </div>

            <div className="mt-[18px] grid grid-cols-3 overflow-hidden rounded-[10px] border border-white/10 bg-white/[0.04]">
              {[
                ['推算位次', '≈ 12,485', true],
                ['超本科线', '+ 110 分', false],
                ['可冲击', '中流 985', false],
              ].map(([label, value, serif]) => (
                <div key={label as string} className="border-r border-white/[0.08] px-4 py-3.5 last:border-r-0">
                  <div className="text-[10px] uppercase tracking-[1.4px] text-white/45">{label}</div>
                  <div className={`mt-1 text-[14px] font-semibold text-white ${serif ? 'font-serif tabular-nums' : ''}`}>
                    {value}
                  </div>
                </div>
              ))}
            ...
            <div className="mt-[18px] flex flex-wrap items-center gap-2">
              <StatusPill type="rush" />
              <StatusPill type="safe" />
              <StatusPill type="stable" />
              <span className="ml-auto text-xs text-white/55">已为你预筛 168 所</span>
            </div>

            <Link href="/recommend" className="btn-willnest btn-accent-willnest mt-[18px] w-full justify-center">
              查看智能推荐方案 →
            </Link>
          </div>
        </div>
      ...
      <section className="border-y border-border-subtle bg-surface py-8">
        <div className="mx-auto flex max-w-[1200px] flex-wrap items-center justify-between gap-4 px-4 text-[12px] uppercase tracking-[1.4px] text-text-muted sm:px-6 lg:px-8">
          <span>数据来源</span>
        ...
        <div className="mx-auto max-w-[1200px]">
          ...
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-3">
            {[
              ...
              <div key={num} className="relative rounded-xl bg-surface p-8 shadow-card">
                <div className="font-serif text-5xl font-semibold leading-none text-accent/50 tabular-nums">{num}</div>
                <h3 className="mt-2 font-serif text-[22px] font-semibold text-text">{title}</h3>
                <p className="mt-2 text-[15px] leading-[1.7] text-text-tertiary">{desc}</p>
                {index < 2 && <span className="absolute right-[-14px] top-1/2 hidden -translate-y-1/2 text-xl text-text-faint md:block">→</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-surface px-4 py-[72px] sm:px-6 lg:px-8">
        <div className="mx-auto max-w-[1200px]">
          <SectionHeader eyebrow="核心能力 · CORE CAPABILITIES" title="一个工作台，覆盖整个填报季。" centered />
          <div className="mt-10 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {features.map((feature) => (
              <Link key={feature.title} href={feature.href} className="rounded-xl bg-surface p-7 text-text no-underline shadow-card transition-all duration-300 hover:-translate-y-px hover:shadow-card-hover">
                <div className={`flex h-12 w-12 items-center justify-center rounded-[10px] ${feature.bg} text-[22px]`}>
                  {feature.icon}
                </div>
                <h3 className="mt-[18px] font-serif text-[20px] font-semibold text-text">{feature.title}</h3>
                <p className="mt-1.5 text-[14px] leading-[1.7] text-text-tertiary">{feature.desc}</p>
                <span className="mt-3.5 inline-flex text-[13px] font-medium text-primary">{feature.action} →</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <div className="mx-auto max-w-[1200px]">
          ...
          <div className="mt-10 grid gap-10 rounded-[14px] bg-surface p-6 shadow-card lg:grid-cols-[1fr_1.2fr] lg:p-8">
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="mt-2 text-[15px] leading-[1.7] text-text-tertiary">
                  综合 4 年录取数据与你勾选的偏好，从 168 所院校中挑选 45 条志愿建议，并按概率和梯度排好顺序。
                </p>
              </div>
              ...
              <Link href="/recommend" className="btn-willnest btn-primary-willnest justify-center">
                查看完整 45 条建议 →
              </Link>
            </div>
            <div className="flex flex-col gap-2">
              ...
              {sampleRows.map((row) => (
                <div key={row.order} className={`grid grid-cols-[32px_48px_minmax(0,1fr)_auto_auto] items-center gap-3 rounded-[10px] border-l-[3px] bg-surface p-3 shadow-card ${row.type === 'rush' ? 'border-l-rush' : row.type === 'safe' ? 'border-l-safe' : 'border-l-stable'}`}>
                  <span className="font-mono text-xs text-text-faint">{row.order}</span>
                  <span className="flex h-12 w-12 items-center justify-center rounded-[10px] bg-primary-fixed font-serif text-xl font-semibold text-primary">{row.crest}</span>
                  <div className="min-w-0">
                    <div className="truncate text-[15px] font-medium text-text">
                      {row.name} <span className="ml-1.5 rounded-full bg-elite-fixed px-2 py-0.5 text-[11px] font-medium text-elite">985</span>
                    </div>
                    <div className="mt-0.5 truncate text-xs text-text-tertiary">{row.major}</div>
                  </div>
                  <StatusPill type={row.type} />
                  <span className={`font-serif text-lg font-semibold tabular-nums ${row.type === 'rush' ? 'text-rush' : row.type === 'safe' ? 'text-safe' : 'text-stable'}`}>{row.chance}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <div className="mx-auto max-w-[1200px]">
          ...
          <div className="relative mt-12 hidden pb-4 md:block">
            <div className="absolute left-0 right-0 top-[58px] h-0.5 bg-[repeating-linear-gradient(90deg,#e8e6dc_0_6px,transparent_6px_12px)]" />
            <div className="grid grid-cols-6 gap-0">
              {timelineItems.map((item) => (
                <TimelineNode key={`${item.date}-${item.title}`} item={item} />
              ))}
            </div>
          </div>
          <div className="mt-10 space-y-5 md:hidden">
            {timelineItems.map((item) => (
              <TimelineNodeMobile key={`${item.date}-${item.title}-mobile`} item={item} />
            ))}
          </div>
          <div className="mt-9 flex items-center gap-4 rounded-lg border border-border-subtle border-l-[3px] border-l-accent bg-surface p-5 text-[13px] text-text-secondary">
            <span className="text-xl">🔔</span>
            <div>每个关键节点提前提醒；出分后使用你的真实成绩重新计算冲稳保，帮助你在提交前完成最后一次梯度自检。</div>
          </div>
        </div>
      </section>

      <section className="px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <div className="relative mx-auto grid max-w-[1200px] gap-8 overflow-hidden rounded-[18px] bg-primary p-8 text-white sm:p-10 lg:grid-cols-[1.2fr_0.8fr] lg:p-[52px]">
          <div className="absolute inset-0 bg-[url('/images/bg-cta.svg')] bg-cover bg-center opacity-70" />
          ...
          <div className="relative self-end text-left lg:text-right">
...
function SectionHeader({
  ...
  return (
    <div className={centered ? 'mx-auto max-w-[680px] text-center' : 'max-w-[680px]'}>
      ...
      <h2 className="mt-2 font-serif text-[30px] font-semibold leading-tight text-text">{title}</h2>
      {children && <p className="mt-3 text-[17px] leading-[1.65] text-text-tertiary">{children}</p>}
    </div>
  );
}
...
function StatusPill({ type }: { type: string }) {
  ...
  return <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium ${config[1]}`}>● {config[0]}</span>;
}

function PlanBar({ label, color, value }: { label: string; color: 'rush' | 'safe' | 'stable'; value: number }) {
  const colors = {
    rush: ['bg-rush', 'text-rush', 'bg-rush-fixed text-rush'],
    safe: ['bg-safe', 'text-safe', 'bg-safe-fixed text-safe'],
    stable: ['bg-stable', 'text-stable', 'bg-stable-fixed text-stable'],
  }[color];

  return (
    <div className="grid grid-cols-[80px_1fr_60px] items-center gap-3">
      <span className={`inline-flex w-fit rounded-full px-3 py-1 text-xs font-medium ${colors[2]}`}>● {label}</span>
      <div className="h-2 overflow-hidden rounded-full bg-surface-dim">
        <div className={`h-full rounded-full ${colors[0]}`} style={{ width: `${value}%` }} />
      </div>
      <span className={`text-right font-serif text-sm font-semibold tabular-nums ${colors[1]}`}>{value}%</span>
    </div>
  );
}
...
function StatCard({
  ...
  return (
    <div className={`rounded-[10px] border-l-[3px] ${borderClass} bg-surface p-5 shadow-card`}>
...
function TimelineNode({ item }: { item: (typeof fallbackTimeline)[number] }) {
  const dotClass =
    item.tone === 'now'
  ...
  return (
    <div className="relative px-3">
      <span className="mb-2 block font-serif text-[13px] font-semibold text-text-secondary tabular-nums">{item.date}</span>
      <div className={`relative z-10 mb-[18px] h-3.5 w-3.5 rounded-full border-2 ${dotClass}`} />
      <span className={`mb-1.5 inline-block rounded px-2 py-0.5 text-[10px] font-medium uppercase tracking-[1.2px] ${pillClass}`}>{item.label}</span>
      <h3 className="font-serif text-[15px] font-semibold text-text">{item.title}</h3>
      <p className="mt-1 text-[12.5px] leading-[1.55] text-text-tertiary">{item.desc}</p>
    </div>
  );
}

function TimelineNodeMobile({ item }: { item: (typeof fallbackTimeline)[number] }) {
  return (
    <div className="rounded-[10px] bg-surface p-5 shadow-card">
      <div className="font-serif text-[13px] font-semibold text-text-secondary tabular-nums">{item.date}</div>
      <h3 className="mt-2 font-serif text-[18px] font-semibold text-text">{item.title}</h3>

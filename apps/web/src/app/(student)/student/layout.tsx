...
const studentNavIconMap: Record<StudentNavKey, typeof AppstoreOutlined> = {
    ...
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <header className="sticky top-0 z-50 h-14 backdrop-blur-xl bg-[rgba(250,249,245,0.92)] shadow-nav">
        <div className="flex items-center justify-between px-4 h-full max-w-[600px] mx-auto lg:max-w-none lg:px-8">
          ...
          >
            <div className="flex items-center gap-1 rounded-full bg-white/35 p-1">
              {STUDENT_NAV_ITEMS.map((item) => {
                const active = isStudentNavActive(pathname, item.href);
                const Icon = studentNavIconMap[item.key];
                ...
                return (
                  <Link
                    ...
                    className={`no-underline inline-flex h-9 items-center gap-1.5 rounded-full px-4 text-[13px] font-medium leading-none transition-colors duration-200 ${
                      active
                        ? 'bg-white text-[var(--willnest-primary,var(--primary))] shadow-card'
                        : 'text-text-secondary hover:bg-white/60 hover:text-[var(--willnest-primary,var(--primary))]'
                    }`}
                  >
                    <Icon aria-hidden className="text-[15px]" />
      ...
      <main className="flex-1 px-4 py-4 pb-20 max-w-[600px] mx-auto w-full lg:max-w-[1200px] lg:px-8 lg:pt-6 lg:pb-6">
        {children}
      </main>
            ...
            return (
              ...
              >
                {item.highlight && !active ? (
                  ...
                  <Icon aria-hidden className={`text-lg ${active ? 'text-primary' : ''}`} />
                )}
                <span className={`text-[10px] leading-tight ${active ? 'font-medium' : ''} ${item.highlight && !active ? 'mt-0.5' : ''}`}>
                  {item.label}
                </span>
              </Link>
            );

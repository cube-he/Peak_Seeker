...
- 用户负责决定：业务方向、功能边界、优先级、最终取舍。
- Codex 负责承担：技术方案、代码实现、测试验证、部署检查、风险提示。
- 讨论功能时，先用通俗话说明：现在的问题是什么、用户会看到什么变化、这次做什么、这次不做什么、怎么证明它做好了。
- 少用技术术语；必须用时，用括号补一句人话解释。
- 每个功能都要先单独商谈细节，再给计划，再执行。
- 遇到业务取舍，停下来问用户；遇到纯技术实现细节，由 Codex 自行判断并说明原因。
- 不要只针对一个例子打补丁，要优先做通用、可复用、可商业化的方案。
- 计划里必须写清楚：改动范围、影响页面/接口、验证方式、残余风险。
- 执行后必须说明：改了什么、验证了什么、哪些没验证或还有风险。

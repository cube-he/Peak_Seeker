# -*- coding: utf-8 -*-
"""
合并28个2023年已校验征集志愿xlsx，按征集次数展开计划数和来源网页。
输出：征集志愿_2023_合并.xlsx（29列统一结构）
...
EXCLUDE_BATCH = {'3335', '3336'}
OUTPUT_PATH = os.path.join(BASE_DIR, '征集志愿_2023_合并.xlsx')
...
def find_source_files():
    """找到所有2023年已校验xlsx（排除3335/3336），按batch_id排序"""
    files = []
    for root, dirs, fnames in os.walk(BASE_DIR):
        for fn in fnames:
            if '2023' in fn and '已校验' in fn and fn.endswith('.xlsx'):
                batch_id = fn.split('_')[0]
                if batch_id not in EXCLUDE_BATCH:
